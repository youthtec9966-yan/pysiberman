...
from PySide6.QtWidgets import QApplication, QSystemTrayIcon, QMenu, QDialog, QFormLayout, QLineEdit, QComboBox, QSpinBox, QDoubleSpinBox, QPushButton, QHBoxLayout, QVBoxLayout, QLabel, QFileDialog, QMessageBox, QPlainTextEdit, QWidget, QProgressBar
...
class RecorderDialog(QDialog):
    def __init__(self, parent=None, device_index=-1):
        super().__init__(parent)
        self.setWindowTitle("录制唤醒词/打断词")
        ...
        self.name_edit.setPlaceholderText("输入词语名称 (如: 停下)")
    ...
    def save_audio(self):
        ...
        try:
            path = Path("templates") / f"{name}.wav"
...
class TemplateManageDialog(QDialog):
    def __init__(self, parent=None, device_index=-1):
        ...
        layout = QVBoxLayout()
        layout.addWidget(QLabel("已录制的指令 (在 templates 目录下):"))
    ...
    def record_new(self):
        dlg = RecorderDialog(self, device_index=self.device_index) 
        dlg.exec()
        self.refresh_list()
    ...
    def _ensure(self):
        ...
        if not self.config_path.exists():
            self.config_path.write_text(json.dumps({
                ...
                "moa_api_url": "https://moa.imtim.cn:8000/api/v2/workflow/invoke",
                "moa_workflow_id": "4cb75554c0164b0aa3c06e26c73fcce9",
                "moa_stream": False
...
class AudioWakeWorkerASR(QObject):
    ...
    def __init__(self, phrases: str, device_index: int, api_key: str, audio_match_enabled: bool = True, audio_match_threshold: float = 0.45):
        ...
        self.matcher = AudioTemplateMatcher()
    ...
    def _update_noise_floor(self, arr):
        ...
        # 计算当前噪音阈值（取历史记录中的最小值）
        self.noise_floor = min(self.vol_history) * 1.2  # 稍微放大以确保捕捉到噪音
    ...
    def _is_speech(self, arr):
        ...
        # 只有当当前最大音量大于噪音阈值加上一定余量，且大于平均最大音量的1.5倍时，才视为有语音
        return current_max > (self.noise_floor + self.NOISE_MARGIN) and current_max > (avg_max_vol * 1.5)
    ...
    def _run(self):
        ...
        try:
            ...
            # 参数配置
            MAX_SILENCE_FRAMES = 25  # 约 800ms 静音判定为结束
            ...
            while self._running:
                try:
                    ...
                    if in_speech:
                        ...
                        if (
                            self._mode == "chat"
                            and self.audio_match_enabled
                            and self.matcher
                            and len(speech_frames) >= MIN_MATCH_FRAMES * 512
                            and len(speech_frames) % (8 * 512) == 0
                        ):
                            window_pcm = speech_frames[-16000:] if len(speech_frames) > 16000 else speech_frames
                            match_name, match_dist = self.matcher.match(window_pcm, threshold=self.audio_match_threshold)
                            if match_name:
                                if match_dist is not None:
                                    self.log.emit(f"Audio Match: {match_name} ({match_dist:.3f})")
                                else:
                                    self.log.emit(f"Audio Match: {match_name}")
                                self.chat_input_detected.emit(match_name)
                                speech_frames = []
                                in_speech = False
                                silence_counter = 0
                                continue
                        ...
                        if silence_counter >= MAX_SILENCE_FRAMES or len(speech_frames) >= MAX_SPEECH_DURATION_FRAMES * 512:
                            ...
                            if len(valid_pcm) > MIN_MATCH_FRAMES * 512:
                                ...
                                if self._mode == "chat" and self.audio_match_enabled and self.matcher:
                                    match_name, match_dist = self.matcher.match(valid_pcm, threshold=self.audio_match_threshold)
                                    if match_name:
                                        if match_dist is not None:
                                            self.log.emit(f"Audio Match: {match_name} ({match_dist:.3f})")
                                        else:
                                        ...
                                        silence_counter = 0
                                        continue
                                    if match_dist is not None and match_dist <= self.audio_match_threshold * 1.5:
                                        self.log.emit(f"Audio Match Miss: {match_dist:.3f} > {self.audio_match_threshold:.3f}")
...
class SettingsDialog(QDialog):
    ...
    2. 唤醒引擎设置 (ASR/VAD)
    3. ASR 模型参数 (大小, 设备, 目录)
    4. MOA (Multi-Agent Orchestration) 工作流配置
    """
    def __init__(self, cfg: ConfigManager, secrets: SecretStore):
        """
        初始化设置界面
        ...
        self.moa_api_url = QLineEdit(self.cfg.get("moa_api_url", "https://moa.imtim.cn:8000/api/v2/workflow/invoke"))
        self.moa_workflow_id = QLineEdit(self.cfg.get("moa_workflow_id", "4cb75554c0164b0aa3c06e26c73fcce9"))
        self.moa_stream = QComboBox()
        self.moa_stream.addItem("False", False)
        self.moa_stream.addItem("True", True)
        self.moa_stream.setCurrentIndex(1 if self.cfg.get("moa_stream", False) else 0)
        
        self.chat_engine_combo = QComboBox()
        self.chat_engine_combo.addItem("LLM (OpenAI/Compatible)", "llm")
        self.chat_engine_combo.addItem("MOA (Multi-Agent)", "moa")
        ce = self.cfg.get("chat_engine", "llm")
        self.chat_engine_combo.setCurrentIndex(1 if ce == "moa" else 0)
        ...
        form = QFormLayout()
        form.addRow("对话引擎", self.chat_engine_combo)
        ...
        form.addRow("MOA API URL", self.moa_api_url)
        form.addRow("MOA Workflow ID", self.moa_workflow_id)
        form.addRow("MOA Stream", self.moa_stream)
        ...
        form.addRow("音频指令", self.audio_match_enabled)
        ...
        self.btn_test.setVisible(True) # Enable LLM test button
        self.btn_test_moa = QPushButton("测试 MOA")
        btn_ok = QPushButton("保存")
        btn_cancel = QPushButton("取消")
        self.btn_test.clicked.connect(self._test_llm_connection)
        self.btn_test_moa.clicked.connect(self._test_moa_connection)
        btn_ok.clicked.connect(self._save)
        btn_cancel.clicked.connect(self.reject)
        btns = QHBoxLayout()
        btns.addWidget(self.btn_test)
        btns.addWidget(self.btn_test_moa)
        btns.addStretch()
    ...
    def _save(self):
        ...
        """
        self.cfg.set("chat_engine", self.chat_engine_combo.currentData())
        ...
        self.cfg.set("moa_api_url", self.moa_api_url.text())
        self.cfg.set("moa_workflow_id", self.moa_workflow_id.text())
        self.cfg.set("moa_stream", self.moa_stream.currentData())
    ...
    def _test_moa_connection(self):
        ...
        4. 解析并显示返回结果
        """
        url = self.moa_api_url.text()
        workflow_id = self.moa_workflow_id.text()
        stream = self.moa_stream.currentData()
        
        original_text = self.btn_test_moa.text()
        self.btn_test_moa.setEnabled(False)
        self.btn_test_moa.setText("测试中...")
        QApplication.processEvents()
        ...
        try:
            ...
            resp_init = requests.post(url, json=payload_init, headers=headers, timeout=10)
            if resp_init.status_code != 200:
                raise Exception(f"初始化失败: {resp_init.status_code}\n{resp_init.text}")
                
            data_init = resp_init.json()
            if data_init.get("status_code") != 200:
                 raise Exception(f"初始化返回错误: {json.dumps(data_init, ensure_ascii=False)}")

            session_id = data_init["data"]["session_id"]
            
            # Find input event to get node_id and message_id
            events = data_init["data"].get("events", [])
            input_event = next((e for e in events if e.get("event") == "input" and e.get("status") == "end"), None)
            
            if not input_event:
                # If no input event, maybe it finished already? Or unexpected state.
                QMessageBox.information(self, "MOA 初始化结果", f"初始化成功，但未找到输入节点。\n{json.dumps(data_init, ensure_ascii=False)}")
                return
            ...
            if not node_id or not message_id:
                raise Exception("初始化响应中缺少 node_id 或 message_id")
            ...
            resp_chat = requests.post(url, json=payload_chat, headers=headers, timeout=30)
            
            if resp_chat.status_code == 200:
                resp_data = resp_chat.json()
                
                # Determine where 'events' is located
                data_node = resp_data
                if isinstance(resp_data, dict) and "data" in resp_data and isinstance(resp_data["data"], dict):
                     data_node = resp_data["data"]
                
                response_text = ""
                found_msg = False
                
                if isinstance(data_node, dict):
                     events = data_node.get("events", [])
                     for evt in events:
                          if isinstance(evt, dict) and "output_schema" in evt:
                               schema = evt["output_schema"]
                               if isinstance(schema, dict) and "message" in schema:
                                    response_text = schema["message"]
                                    found_msg = True
                                    if evt.get("event") == "stream_msg":
                                         break
                ...
                if not found_msg:
                    try:
                        # Fallback logic
                        if isinstance(data_node, dict):
                            if "outputs" in data_node:
                                outputs = data_node["outputs"]
                                response_text = outputs.get("answer") or outputs.get("text") or str(outputs)
                            elif "answer" in data_node:
                                response_text = data_node["answer"]
                            elif "result" in data_node:
                                response_text = data_node["result"]
                            else:
                                longest_val = ""
                                for k, v in data_node.items():
                                    if isinstance(v, str) and len(v) > len(longest_val):
                                        longest_val = v
                                if len(longest_val) > 0:
                                    response_text = longest_val
                                else:
                                    response_text = json.dumps(resp_data, ensure_ascii=False)
                        else:
                            response_text = str(resp_data)
                    except Exception:
                        response_text = resp_chat.text
                
                QMessageBox.information(self, "MOA 测试结果", f"成功: {resp_chat.status_code}\n回复内容: {response_text}\n\n已切换为 MOA 模式")
                self.cfg.set("chat_engine", "moa")
            else:
                QMessageBox.warning(self, "MOA 测试失败", f"状态码: {resp_chat.status_code}\n{resp_chat.text[:1000]}")
                
        except Exception as e:
            QMessageBox.critical(self, "MOA 测试出错", f"错误: {str(e)}")
        finally:
            self.btn_test_moa.setEnabled(True)
            self.btn_test_moa.setText(original_text)
...
class TrayApp(QApplication):
    ...
    def __init__(self, argv):
        ...
        self.act_audio_cmd = QAction("管理音频指令")
        ...
        menu.addAction(self.act_audio_cmd)
        ...
        self.act_audio_cmd.triggered.connect(self.open_audio_manager)
        ...
        self.moa_session = None # {session_id, message_id, node_id}
        ...
        self.tts_voice = "zh-CN-YunyangNeural"
        self.tts_rate = "+10%"
        self.tts_volume = "+0%"
        self.tts_pitch = "+0Hz"
    ...
    def open_audio_manager(self):
        ...
        dlg = TemplateManageDialog(None, device_index=device_index)
        dlg.exec()
        # Reload templates in worker if exists
        if self.worker and self.worker.matcher:
            self.worker.matcher.reload_templates()
    ...
    def open_settings(self):
        ...
        try:
            dlg = SettingsDialog(self.cfg, self.secrets)
    ...
    def start_listening(self):
        ...
        self.worker.volume.connect(self.ctrl.update_volume)
        self.worker.start()
        self.act_start.setEnabled(False)
        self.act_stop.setEnabled(True)
        self._on_log("开始监听")
    ...
    def handle_backend_asr(self, text: str):
        ...
        if self.is_speaking:
            # 仅对明确停止词触发打断，避免把“小石/警官”等普通词误判成打断
            clean_text = "".join(c for c in text if c.isalnum() or '\u4e00' <= c <= '\u9fff')
            interrupt_keywords = ["停", "停下", "别说", "闭嘴", "安静", "打断", "不听"]
            
            if any(k in clean_text for k in interrupt_keywords):
        ...
        # 仅在非说话状态把 ASR 文本上屏，避免 TTS 回声持续干扰聊天面板
        try:
        ...
        if hasattr(self, 'ws_loop') and self.ws_loop and self.ws_loop.is_running():
            asyncio.run_coroutine_threadsafe(self.ask_llm(text), self.ws_loop)
        else:
    ...
    def _play_tts_mp3_backend(self, mp3_path: Path) -> bool:
        try:
    ...
    def _handle_tts_request(self, order_index: int, req_id: str, tts_text: str):
        ...
        try:
            ...
            with self.tts_order_cond:
                while order_index != self.tts_play_index:
                    if self.stop_generation and order_index < self.tts_play_index:
                        self.ws_broadcast({"type": "TTS_AUDIO", "req_id": req_id, "url": "", "text": text, "backend_played": False})
                        should_advance = False
                        return
            ...
            played = self._play_tts_mp3_backend(mp3_path)
                ...
                try:
                    ...
                    if data.get("type") == "ASR_RESULT" and data.get("text"):
                        ...
                        try:
                            ...
                            pass
                        asyncio.create_task(self.ask_llm(data["text"]))
                        ...
                        asyncio.create_task(asyncio.to_thread(self._handle_tts_request, order_index, req_id, tts_text))
                except Exception as e:
    ...
    async def ask_llm(self, text: str, hidden_input: bool = False):
        ...
        流程:
        1. 检查并初始化 MOA 会话
        2. 构造 MOA 请求 Payload (带 session_id, node_id)
        3. 发送 HTTP POST 请求
        4. 解析响应，提取回复内容
        5. 更新聊天历史并广播给前端
        """
        ...
        async with self.llm_lock:
            ...
            try:
                self._file_log(f"moa ask (hidden={hidden_input})")
            except Exception:
                pass
            ...
            engine = self.cfg.get("chat_engine", "llm")
            if engine == "llm":
                ...
                try:
                    ...
                    if not hidden_input:
                        self.chat_history.append({"role": "user", "content": text})
                        # self.ws_broadcast({"type": "CHAT_APPEND", "role": "user", "text": text})

                    response = await client.chat.completions.create(
                        model=model,
                        messages=messages,
                        max_tokens=500, # 限制回答长度，配合 prompt
                        stream=True
                    )
                    ...
                    async for chunk in response:
                        ...
                        if content:
                            ...
                            buffer += content
                            # Check for sentence endings to split TTS chunks
                            if any(p in content for p in "。！？；.!?;"):
                                # Send buffer as TTS chunk
                                self.ws_broadcast({"type": "TTS_CHUNK", "text": buffer})
                                buffer = ""
                            
                            # Send partial text for UI update
                            if first_chunk:
                                self.ws_broadcast({"type": "CHAT_START", "role": "assistant", "text": ""})
                                first_chunk = False
                            self.ws_broadcast({"type": "CHAT_PARTIAL", "text": content})
                    
                    # Send any remaining buffer (only if not interrupted, or maybe still send it?)
                    ...
                    # self.ws_broadcast({"type": "CHAT_APPEND", "role": "assistant", "text": full_content})
            ...
            moa_url = self.cfg.get("moa_api_url")
            workflow_id = self.cfg.get("moa_workflow_id")
            stream = self.cfg.get("moa_stream")
            
            if not moa_url or not workflow_id:
                self.ws_broadcast({"type": "CHAT_APPEND", "role": "assistant", "text": "请先在设置中填写 MOA API 相关信息"})
                return
            
            # Ensure session exists
            if not self.moa_session:
                 await asyncio.to_thread(self._init_moa_session)
                 if not self.moa_session:
                      self.ws_broadcast({"type": "CHAT_APPEND", "role": "assistant", "text": "无法连接到 MOA 服务"})
                      return
            ...
            try:
                ...
                session_id = self.moa_session.get("session_id")
                message_id = self.moa_session.get("message_id")
                node_id = self.moa_session.get("node_id")
                
                # Robust type conversion to prevent "expected string, got list" errors
                if isinstance(session_id, list):
                    session_id = str(session_id[0]) if session_id else ""
                elif not isinstance(session_id, str) and session_id is not None:
                    session_id = str(session_id)

                if isinstance(message_id, list):
                    message_id = str(message_id[0]) if message_id else ""
                elif not isinstance(message_id, str) and message_id is not None:
                    message_id = str(message_id)

                if isinstance(node_id, list):
                     node_id = str(node_id[0]) if node_id else ""
                elif not isinstance(node_id, str) and node_id is not None:
                     node_id = str(node_id)

                self._file_log(f"Preparing MOA payload: session={session_id}, node={node_id}, msg={message_id}")
                ...
                def do_request():
                    return requests.post(moa_url, json=payload, headers=headers, timeout=30)
                ...
                if response.status_code == 200:
                    try:
                        ...
                        # Determine where 'events' and 'session_id' are located
                        # Support both wrapped in "data" or direct dictionary
                        data_node = resp_data
                        if isinstance(resp_data, dict) and "data" in resp_data and isinstance(resp_data["data"], dict):
                             data_node = resp_data["data"]

                        # 1. Update session info for next turn
                        if isinstance(data_node, dict):
                             events = data_node.get("events", [])
                             
                             # Log event summary for debugging
                             try:
                                 event_summary = [f"{e.get('event')}:{e.get('status')}" for e in events if isinstance(e, dict)]
                                 self._file_log(f"MOA Response Events: {event_summary}")
                             except Exception:
                                 pass

                             next_input = next((e for e in events if e.get("event") == "input" and e.get("status") == "end"), None)
                             if next_input:
                                  self.moa_session["node_id"] = next_input.get("node_id")
                                  self.moa_session["message_id"] = next_input.get("message_id")
                                  self._file_log(f"MOA session updated: node={next_input.get('node_id')} msg={next_input.get('message_id')}")
                             else:
                                  self._file_log("MOA response has no next input event - Resetting session for next turn")
                                  self.moa_session = None
                             
                             if self.moa_session and data_node.get("session_id"):
                                  self.moa_session["session_id"] = data_node.get("session_id")
                        ...
                        if isinstance(data_node, dict):
                             ...
                             for evt in events:
                                  if isinstance(evt, dict) and "output_schema" in evt:
                                       schema = evt["output_schema"]
                                       if isinstance(schema, dict) and "message" in schema:
                                           msg_val = schema["message"]
                                           if isinstance(msg_val, list):
                                               response_text = "".join([str(x) for x in msg_val])
                                           else:
                                               response_text = str(msg_val)
                                           found_msg = True
                                           # If we found a stream_msg with message, that's likely the one we want
                                           if evt.get("event") == "stream_msg":
                                                break
                    ...
                    response_text = f"MOA Error {response.status_code}: {response.text}"
                    # If session expired or invalid, maybe clear it so next time we re-init
                ...
                if response_text:
                    ...
                    self.ws_broadcast({"type": "CHAT_APPEND", "role": "assistant", "text": clean_text})
            except Exception as e:
                error_message = f"请求 MOA 时出错: {str(e)}"
                self._on_error(error_message)
                # Suppress only the specified timeout error
                if "HTTPSConnectionPool(host='moa.imtim.cn', port=8000): Read timed out. (read timeout=30)" not in error_message:
                    self.ws_broadcast({"type": "CHAT_APPEND", "role": "assistant", "text": error_message})

    def _init_moa_session(self):
        """
        初始化 MOA 会话
        
        通过发送一个初始请求获取 session_id 和初始 input event (node_id, message_id)。
        这是后续对话的基础。
        """
        try:
            if self.moa_session:
            ...
            resp = requests.post(moa_url, json=payload, headers=headers, timeout=10)
            if resp.status_code == 200:
                ...
                if data.get("status_code") == 200 and "data" in data:
                    ...
                    if session_id and input_event:
                        self.moa_session = {
                            "session_id": session_id,
                            "node_id": input_event.get("node_id"),
                            "message_id": input_event.get("message_id")
                        }
                        self._on_log(f"MOA session initialized: {session_id}")
                    else:
                        self._on_error("MOA init failed: missing session_id or input event")
                else:
                     self._on_error(f"MOA init failed: {data.get('status_message')}")
            else:
                self._on_error(f"MOA init http error: {resp.status_code}")
        except Exception as e:
            self._on_error(f"MOA init exception: {e}")
        ...
        try:
            if self.player_active:
                ...
                try:
                    ...
                    pid_ok = False
                if (not pid_ok) and (len(self.ws_clients) == 0):
                    self.player_active = False
                    self.browser_pid = None
                    try:
    ...
    def manual_wake(self):
        ...
        2. 广播 "唤醒成功"
        3. 向 MOA 发送静默指令 "你好，请你介绍一下你自己" 以触发开场白
        """
        ...
        try:
            ...
            # 优先检查 WebSocket 连接（最准确的活体检测）
            if self.ws_clients and len(self.ws_clients) > 0:
                self._on_log("检测到数字人连接活跃 (WebSocket在线)，不启动新窗口。")
    ...
    def launch_player(self):
        ...
        if exe:
            try:
                ...
                # 添加 --new-window 确保新窗口启动，避免合并到现有 Edge 进程导致参数失效
                # 改为窗口模式启动 (--app=URL)，去除 --kiosk 和 --edge-kiosk-type=fullscreen
                # 增加 --autoplay-policy=no-user-gesture-required 以允许自动播放音频
                # 增加 --use-fake-ui-for-media-stream 以自动允许麦克风/摄像头权限
                # 增加 --no-first-run 避免启动时的欢迎页干扰
                args = [exe, "--app=" + url, "--autoplay-policy=no-user-gesture-required", "--use-fake-ui-for-media-stream", "--no-first-run"]
