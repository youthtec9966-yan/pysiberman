...
class TrayApp(QApplication):
    ...
    def __init__(self, argv):
        ...
        self.tts_cache_dir = self.runtime_data_dir / "tts_cache"
        self.tts_lock = threading.Lock()
        self.tts_key_locks = {}
        self.tts_key_locks_lock = threading.Lock()
    ...
    def _try_close_player_after_answer(self):
        if not self.close_after_answer:
            return
    ...
    def handle_audio_interrupt_command(self, command_name: str):
        ...
        try:
            ...
            pass
        if self.ws_clients and was_speaking:
            text = "嗯"
    ...
    def handle_backend_asr(self, text: str):
        ...
        force_interrupt_after_wake = wake_grace_active and len(clean_text) >= 2
    ...
    def _is_single_turn_locked(self) -> bool:
        return bool(self.close_after_answer)
    ...
    def _run_startup_diagnostics(self):
        ...
        try:
            tts_file = self._synthesize_tts_edge_file("连接测试")
            tts_ok = bool(tts_file and tts_file.exists())
            self._on_log(f"启动自检: EdgeTTS={'OK' if tts_ok else 'FAIL'}")
        except Exception as e:
            self._on_log(f"启动自检: EdgeTTS检测异常 {e}")
    ...
    def clear_tts_cache(self) -> tuple[int, int]:
        ...
        try:
            with self.tts_lock:
                for p in self.tts_cache_dir.glob("tts_*.mp3"):
                    try:
                        p.unlink()
                        removed += 1
                    except Exception:
    ...
    def _get_tts_key_lock(self, key: str):
        with self.tts_key_locks_lock:
            lk = self.tts_key_locks.get(key)
            if lk is None:
                lk = threading.Lock()
                self.tts_key_locks[key] = lk
            return lk
    ...
    def _synthesize_tts_edge_file(self, text: str) -> Optional[Path]:
        try:
            ...
            voice_sig = "|".join([
                t,
                str(self.tts_voice or ""),
                str(self.tts_rate or ""),
                str(self.tts_volume or ""),
                str(self.tts_pitch or ""),
                "v2"
            ])
            key = hashlib.md5(voice_sig.encode("utf-8")).hexdigest()[:16]
            mp3_path = self.tts_cache_dir / f"tts_{key}.mp3"
            if mp3_path.exists() and mp3_path.stat().st_size > 256:
                return mp3_path
            with self._get_tts_key_lock(key):
                if mp3_path.exists() and mp3_path.stat().st_size > 256:
                    return mp3_path
    ...
    async def _process_streaming_tts(self, req_id: str, text: str, cancel_seq: int):
        ...
        try:
            async for message in stream:
                ...
                if message.get("type") != "audio":
                    continue
                b64 = base64.b64encode(message["data"]).decode("utf-8")
                self.ws_broadcast({"type": "TTS_STREAM_CHUNK", "req_id": req_id, "chunk": b64})
        except Exception as e:
            self._on_log(f"TTS Stream Error: {e}")
        finally:
            try:
                await stream.aclose()
            except Exception:
    ...
    def _handle_tts_request(self, order_index: int, req_id: str, tts_text: str, cancel_seq: int):
        ...
        try:
            ...
            asyncio.run(self._process_streaming_tts(req_id, text, cancel_seq))
